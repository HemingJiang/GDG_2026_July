...
def decide(question: str, tools: list[dict], history: list[dict]) -> dict:
    ...
    or
        {"kind": "final", "text": ..., "ranked": [...], "stop_reason": str}

    `ranked` is a pure function of (question, tools) — history does not affect a
    single score, so ranked[0] is IDENTICAL on every step of a run. Only the
    loop below reads history. That is why the stop needs its own explanation:
    `stop_reason` names the branch we actually left on, so a trace can say what
    stopped the loop instead of guessing from the scores.
    ...
    if not ranked:
        stop_reason = "no tools were provided"
    else:
        stop_reason = (
            f"nothing cleared the bar of {MIN_DOC_MATCHES}+ shared docstring words "
            f"and score >= {MIN_SCORE_TO_CALL_TOOL:.1f}"
        )
    ...
    for candidate in ranked:
        ...
        # Each one records WHY, because "it scored well" is never the reason a
        # second step doesn't happen — the score is the same as it was in step 1.
        #
        # 1. Don't repeat an identical call. Its answer is already in history.
        if already_observed(candidate["name"], args, history):
            stop_reason = f"{candidate['name']}({args}) was already answered"
            continue
        ...
        if have_result and not used_observation:
            stop_reason = (
                f"nothing left to chain: {candidate['name']} would re-read the "
                f"question, not the result"
            )
            break
